...
`_data/projects.yml` is the source of truth for project facts. It drives the work-list rendering on `index.html`, `projects.html`, `zh/index.html`, and `zh/projects.html` via `{% for project in site.data.projects %}`. Schema per entry: `slug`, `title`, bilingual `years.{en,zh}`, bilingual `tags.{en,zh}`, `status` + bilingual `status_label.{en,zh}`, `release`, `release_source` (`public_tag` / `private_main` / `local_tag`), bilingual `snapshot_label.{en,zh}` (private entries only), `repo_url`, `crate_url`, `docs_url`, `detail_url`, `zh_detail_url`, `public_source` (boolean — when **false**, templates must not render a "Source" link, and `project-summary.html` renders a **Snapshot** row from `snapshot_label` instead of a "Release" row, so no page makes a private release claim). Prose lives in the page templates; the data file is facts only.
